...
void Update_Odometry() {
    ...
    double sr = distancePerPulse * (encoderRightPulses - encoderRPosPrev);
    double sl = distancePerPulse * (encoderLeftPulses - encoderLPosPrev);

    double srd = encoderRightPulses - encoderRPosPrev;
    double sld = encoderLeftPulses - encoderLPosPrev;

    encoderRPosPrev = encoderRightPulses;
    encoderLPosPrev = encoderLeftPulses;

    if(abs(srd) - abs(sld) == 0) {
	if(srd == sld) {
